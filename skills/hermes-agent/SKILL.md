...
### Files attached via API server not on disk

When a user uploads a file through a messaging platform or the API server, the message text includes `[File: filename.ext]` but the actual file content is NOT persisted to the filesystem — it's just a textual reference in the chat message. Do NOT search for it with `find`, `ls`, or `search_files`. Instead, ask the user for the actual file path on disk, or to paste the content directly.

Pitfall: the `read_file` tool can return "Cannot read binary file" for a path that does NOT actually exist — it checks the file extension (.zip, .png, etc.) before checking existence on disk. Do not take this error as confirmation the file exists.
